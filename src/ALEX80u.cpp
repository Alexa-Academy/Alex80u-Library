...
void ALEX80u::begin_MCP() {
    ...
    mcp1.begin(10, this->spiBus, this->ms);
    mcp2.begin(9, this->spiBus, this->ms);
...
#if defined(ARDUINO_AVR_UNO)
    return static_cast<uint8_t>((PIND >> 3) & 0x1fU) | static_cast<uint8_t>((PINC & 0x38U) << 2);
#elif defined(ARDUINO_MINIMA)
...
void ALEX80u::write_DATA(uint8_t val) {
    ...
    noInterrupts();
    PORTD = (PORTD & static_cast<uint8_t>(~AVR_DATA_PORTD_MASK)) |
    static_cast<uint8_t>((val & 0x1fU) << 3);
    PORTC = (PORTC & static_cast<uint8_t>(~AVR_DATA_PORTC_MASK)) |
